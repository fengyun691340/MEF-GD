...
class MFIAdapter(nn.Module):
    def __init__(
        ...
        self.input_hint_block = TimestepEmbedSequential(
            conv_nd(dims, local_channels, 16, 3, padding=1),
            nn.SiLU(),
    ...
    def forward(self, x, timesteps, context, local_conditions, **kwargs):
        ...
        guided_hint = self.input_hint_block(local_conditions, emb, context)
        ...
        for layer_idx, (module, zero_conv) in enumerate(zip(self.input_blocks, self.zero_convs)):
            if guided_hint is not None:
                ...
                h += guided_hint
                guided_hint = None
            elif layer_idx in self.inject_layers:
                h = module(h, emb, context, local_features[self.inject_layers.index(layer_idx)])
            else:
                h = module(h, emb, context)
            outs.append(zero_conv(h, emb, context))
...
class CGAFusion(nn.Module):
    ...
    def forward(self, x, y):
        ...
        return result
class CGAControlUNetModel(UNetModel):
    '''
    def __init__(self, *args, **kwargs):  
        super().__init__(*args, **kwargs)  # 调用父类的初始化方法  
        block_dims = [1280, 1280, 1280,1280,1280,640,640,640,320,320,320,320]
        ##self.blocks = nn.ModuleList([CGAFusion(dim) for dim in block_dims])     
        ##self.block = CGAFusion(1280)  # 在实例方法中初始化 self.block 
        self.block = CGAFusion(1280)  # 在实例方法中初始化 self.block 
        self.blocks = nn.ModuleList([CGAFusion(dim) for dim in block_dims])
    def forward(self, x, timesteps=None, context=None, local_control=None, **kwargs):
        ...
        #h += local_control.pop()
        h = self.block(h, local_control.pop())
        ...
        for i, module in enumerate(self.output_blocks):
            block = self.blocks[i]  
                # 假设hs和control都有足够的元素，弹出对应索引的元素  
            hcat = block(hs.pop(), local_control.pop())
            h=torch.cat([h,hcat],dim=1)
